...
import { WorkspaceConfig, EarthbarStore } from './earthbarStore'
import { sortByField } from './util'
import { logEarthbarPanel } from './log'
...
export class EarthbarWorkspacePanel extends React.Component<
  EbPanelProps,
  EbWorkspacePanelState
> {
  ...
  handleAddPub () {
    ...
    if (newPub.length > 0) {
      if (newPub.startsWith('http://') || newPub.startsWith('https://')) {
        const match = newPub.match(
          /https:\/\/api-([a-f0-9]+)\.([^.]+)\.hex\.camp#(.*)$/
        )
        if (match) {
          const proxyPub =
            `${location.origin}/proxy/https/` +
  ...
  render () {
    ...
    return (
      <div className='stack unselectable' style={sWorkspacePanel}>
        {/* current workspace details */}
        {store.currentWorkspace === null ? null : (
          <div className='stack'>
            ...
            <div className='stack'>
              ...
              {store.kit === null
                ...
                : Object.entries(store.kit.syncers).map(pair => {
                    ...
                    return (
                      <div key={domain} className='flexRow selectable'>
                        ...
                        <button
                          className='flexItem linkButton unselectable'
              ...
              >
                <input
                  type='text'
                  className='flexItem flexGrow1 monospace'
                  placeholder='http://...'
                  value={this.state.newPubInput}
                  onChange={e => this.setState({ newPubInput: e.target.value })}
                />
                <button className='button flexItem' type='submit'>
                  Add
                </button>
              </form>
              {store.kit === null ||
              Object.keys(store.kit.syncers).length === 0 ? (
        ...
        <div className='stack indent'>
          {allWorkspaces.map(wsConfig => {
            ...
            let style: React.CSSProperties = isCurrent
              ...
              : {}
            return (
              <div key={wsConfig.workspaceAddress} className='flexRow'>
                <a
                  href='#'
                  style={style}
                  className='flexItem flexGrow1 flexItemVerticalCenter linkButton monospace wrappyText'
                  onClick={e => store.switchWorkspace(wsConfig)}
                >
                  {wsConfig.workspaceAddress}
                </a>
                <button
                  className='flexItem linkButton'
                  onClick={() =>
                    store.removeWorkspace(wsConfig.workspaceAddress)
                  }
                >
                  &#x2715;
                </button>
        ...
        >
          ...
          {this.state.newWorkspaceError === null ? null : (
            <div>{this.state.newWorkspaceError}</div>
          )}
          <div className='faint'>
            A workspace has two parts: a word up to 15 letters long, and a
            random part that's hard to guess. If you're creating a new
            workspace, just make up the random part by mashing the keyboard.
          </div>
          <div className='right'>
            <button
              className='button'
